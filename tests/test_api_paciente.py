...
class TestPaciente(TestFlaskBase):
    ...
    def test_atualiza_um_paciente(self):
        response = self.client.post(url_for('paciente.paciente_novo'), json=self.paciente_json, headers=self.token)
        response = self.client.post(url_for('paciente.paciente_atualizar', cpf=self.paciente_alterado['cpf']), json=self.paciente_alterado, headers=self.token)
        self.assertEqual(response.status_code, 200)
        
    def test_atualiza_um_paciente_com_payload_vazio(self):
        paciente_vazio = {}
        response = self.client.post(url_for('paciente.paciente_novo'), json=self.paciente_json, headers=self.token)
        response = self.client.post(url_for('paciente.paciente_atualizar', cpf=self.paciente_alterado['cpf']), json=paciente_vazio, headers=self.token)
        self.assertEqual(response.status_code, 400)
    ...
    def test_busca_um_paciente_por_cpf(self):
        response = self.client.post(url_for('paciente.paciente_novo'), json=self.paciente_json, headers=self.token)
        response = self.client.get(url_for('paciente.paciente_cpf', cpf=self.paciente_json['cpf']), headers=self.token)
        # import ipdb; ipdb.set_trace()
        self.assertEqual(response.status_code, 200)
    ...
    def test_busca_um_paciente_por_nome(self):
        response = self.client.post(url_for('paciente.paciente_novo'), json=self.paciente_json, headers=self.token)
        response = self.client.get(url_for('paciente.paciente_nome', nome=self.paciente_json['nome']), headers=self.token)
        # import ipdb; ipdb.set_trace()
        self.assertEqual(response.status_code, 200)
        
    def test_busca_um_paciente_por_parte_do_nome(self):
        response = self.client.post(url_for('paciente.paciente_novo'), json=self.paciente_json, headers=self.token)
        response = self.client.get(url_for('paciente.paciente_nome', nome='Pacien'), headers=self.token)
        # import ipdb; ipdb.set_trace()
        self.assertEqual(response.status_code, 200)
    ...
    def test_busca_um_paciente_por_nome_completo(self):
        response = self.client.post(url_for('paciente.paciente_novo'), json=self.paciente_json, headers=self.token)
        response = self.client.get(url_for('paciente.paciente_nome_completo', nome=self.paciente_json['nome']), headers=self.token)
        # import ipdb; ipdb.set_trace()
        self.assertEqual(response.status_code, 200)
